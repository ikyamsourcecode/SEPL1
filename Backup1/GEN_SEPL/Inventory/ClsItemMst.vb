...
#End Region
    ...
    Sub CreateForm()
        Try
            oUtilities.SAPXML("GEN_ITM_MST.xml")
            objForm = oApplication.Forms.GetForm("GEN_ITM_MST", oApplication.Forms.ActiveForm.TypeCount)
            oDBs_Head = objForm.DataSources.DBDataSources.Item("@GEN_ITM_MST")
    ...
    Function Validation(ByVal FormUID As String) As Boolean
        Try
            ...
            If Trim(objForm.Items.Item("code").Specific.Value) = "" Then
                oApplication.StatusBar.SetText("Please enter Item Master", SAPbouiCOM.BoMessageTime.bmt_Short, SAPbouiCOM.BoStatusBarMessageType.smt_Error)
                Return False
                Exit Function
            End If
            oRecordSet = oCompany.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
            oRecordSet.DoQuery("Select Code From [@GEN_ITM_MST] Where Code = '" + Trim(objForm.Items.Item("code").Specific.Value) + "' And Code <> '" + Trim(objForm.Items.Item("code").Specific.Value) + "'")
            If oRecordSet.RecordCount > 0 Then
                oApplication.StatusBar.SetText("Item Master already exists in the database", SAPbouiCOM.BoMessageTime.bmt_Short, SAPbouiCOM.BoStatusBarMessageType.smt_Error)
                Return False
                Exit Function
            End If
            ...
            If pVal.BeforeAction = False Then
                ...
                Select Case pVal.MenuUID
                    Case "GEN_ITM_MST"
                        Me.CreateForm()
                    Case "1282"
                        If oForm.TypeEx = "GEN_ITM_MST" Then
