...
#End Region
    ...
    Sub CreateForm()
        Try
            oUtilities.SAPXML("GEN_CUST_CODE.xml")
            objForm = oApplication.Forms.GetForm("GEN_CUST_CODE", oApplication.Forms.ActiveForm.TypeCount)
            oDBs_Head = objForm.DataSources.DBDataSources.Item("@GEN_CUST_CODE")
    ...
    Function Validation(ByVal FormUID As String) As Boolean
        Try
            ...
            oRecordSet.DoQuery("Select Code From [@GEN_CUST_CODE] Where Code = '" + Trim(objForm.Items.Item("code").Specific.Value) + "' And Code <> '" + Trim(objForm.Items.Item("code").Specific.Value) + "'")
            If oRecordSet.RecordCount > 0 Then
                oApplication.StatusBar.SetText("Customer Code already exists in the database", SAPbouiCOM.BoMessageTime.bmt_Short, SAPbouiCOM.BoStatusBarMessageType.smt_Error)
                Return False
                Exit Function
            End If
            ...
            If pVal.BeforeAction = False Then
                ...
                Select Case pVal.MenuUID
                    Case "GEN_CUST_CODE"
                        Me.CreateForm()
                    Case "1282"
                        If oForm.TypeEx = "GEN_CUST_CODE" Then
