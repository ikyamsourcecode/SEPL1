...
#End Region
    ...
    Sub CreateForm(ByVal FormUID As String)
        Try
            oUtilities.SAPXML("GEN_UNIT_MST.xml")
            objForm = oApplication.Forms.GetForm("GEN_UNIT_MST", oApplication.Forms.ActiveForm.TypeCount)
            oDBs_Head = objForm.DataSources.DBDataSources.Item("@GEN_UNIT_MST")
            oDBs_Detail = objForm.DataSources.DBDataSources.Item("@GEN_UNIT_MST_D0")
    ...
    Sub SetNewLine(ByVal FormUID As String, ByVal Row As Integer, ByRef oMatrix As SAPbouiCOM.Matrix)
        Try
            ...
            oDBs_Detail.SetValue("u_inwhs", oDBs_Detail.Offset, "")
            oDBs_Detail.SetValue("u_outwhs", oDBs_Detail.Offset, "")
            oDBs_Detail.SetValue("u_stwhs", oDBs_Detail.Offset, "")
        ...
        Try
            If pVal.BeforeAction = False Then
                ...
                Select Case pVal.MenuUID
                    ...
                    Case "1282"
                        If objForm.TypeEx = "GEN_UNIT_MST" Then
                            Dim oRSet As SAPbobsCOM.Recordset = oCompany.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
                            oRSet.DoQuery("Select Count(*) + 1 AS 'Count' From [@GEN_UNIT_MST]")
                            oDBs_Head = objForm.DataSources.DBDataSources.Item("@GEN_UNIT_MST")
                            oDBs_Head.SetValue("Code", 0, oRSet.Fields.Item("Count").Value)
                        ...
                        If objForm.TypeEx = "GEN_UNIT_MST" Then
                            If ITEM_ID.Equals("mtx") = True Then
                                ...
                                For Row As Integer = 1 To objMatrix.VisualRowCount
                                    ...
                                    oDBs_Detail.SetValue("u_inwhs", oDBs_Detail.Offset, objMatrix.Columns.Item("inwhs").Cells.Item(Row).Specific.value)
                                    oDBs_Detail.SetValue("u_outwhs", oDBs_Detail.Offset, objMatrix.Columns.Item("outwhs").Cells.Item(Row).Specific.value)
                                    oDBs_Detail.SetValue("u_stwhs", oDBs_Detail.Offset, objMatrix.Columns.Item("stwhs").Cells.Item(Row).Specific.value)
