...
#End Region
    ...
    Sub CreateForm(ByVal FormUID As String)
        Try
            oUtilities.SAPXML("GEN_OTP.xml")
            objForm = oApplication.Forms.GetForm("GEN_OTP", oApplication.Forms.ActiveForm.TypeCount)
            oDBs_Head = objForm.DataSources.DBDataSources.Item("@GEN_OTP")
            objForm.EnableMenu("1288", False)
            objForm.EnableMenu("1289", False)
            objForm.EnableMenu("1290", False)
            objForm.EnableMenu("1291", False)
            objForm.EnableMenu("1294", False)
    ...
    Sub SetNewLine(ByVal FormUID As String, ByVal Row As Integer, ByRef oMatrix As SAPbouiCOM.Matrix)
        Try
            ...
            oDBs_Detail.SetValue("U_iprice", oDBs_Detail.Offset, "")
            ...
            Select Case pVal.EventType
                ...
                Case SAPbouiCOM.BoEventTypes.et_LOST_FOCUS
                    If pVal.ItemUID = "mtx" And pVal.ColUID = "per" And pVal.BeforeAction = False And pVal.Row > 0 Then
                        ...
                        End If
                    End If
                    If pVal.ItemUID = "mtx" And pVal.ColUID = "totqty" And pVal.BeforeAction = False And pVal.Row > 0 Then
                        Dim Total As Decimal = 0
                        For i As Integer = 1 To objMatrix.VisualRowCount
                            ' Total = 0
                            Total = Total + (CDbl(objMatrix.Columns.Item("iprice").Cells.Item(i).Specific.value) * CDbl(objMatrix.Columns.Item("totqty").Cells.Item(i).Specific.value))

                        Next
                        objForm.Items.Item("tprice").Specific.value = Total
                    End If
                ...
                Case SAPbouiCOM.BoEventTypes.et_ITEM_PRESSED
                    ...
                    If pVal.ItemUID = "1" And pVal.BeforeAction = True And (pVal.FormMode = SAPbouiCOM.BoFormMode.fm_ADD_MODE Or pVal.FormMode = SAPbouiCOM.BoFormMode.fm_UPDATE_MODE) Then
                        ...
                        'For i As Integer = 1 To objMatrix.VisualRowCount
                        '    ' Total = 0
                        '    Total = Total + objMatrix.Columns.Item("iprice").Cells.Item(i).Specific.value
                        ...
                        If Not (oDT Is Nothing) And pVal.FormMode <> SAPbouiCOM.BoFormMode.fm_FIND_MODE Then
                            ...
                            If oCFL.UniqueID = "RITCFL" Then
                                ...
                                For i As Integer = 0 To oDT.Rows.Count - 1
                                    ...
                                    oRSet.DoQuery("Select Invntryuom,LastPurPrc From OITM Where ItemCode = '" + Trim(oDT.GetValue("ItemCode", i)) + "'")
                                    oDBs_Detail.SetValue("u_uom", oDBs_Detail.Offset, oRSet.Fields.Item("InvntryUOM").Value)
                                    oDBs_Detail.SetValue("U_iprice", oDBs_Detail.Offset, oRSet.Fields.Item(1).Value)
                                    ' oDBs_Detail.SetValue("U_price", oDBs_Detail.Offset, 100)
                            ...
                            If oCFL.UniqueID = "PRCCFL" Then
                                ...
                                For i As Integer = 0 To oDT.Rows.Count - 1
                                    ...
                                    oDBs_Detail.SetValue("u_iprice", oDBs_Detail.Offset, objMatrix.Columns.Item("iprice").Cells.Item(pVal.Row).Specific.value)
        ...
        Try
            If pVal.BeforeAction = False Then
                ...
                Select Case pVal.MenuUID
                    ...
                    Case "1287"
                        If objForm.TypeEx = "GEN_CUST_BOM" Then
                            Dim DDocnum As Integer
                            DDocnum = objForm.Items.Item("docnum").Specific.value
                            Me.SetDefault(objForm.UniqueID)
                            objMatrix = objForm.Items.Item("mtx").Specific
                            oDBs_Head = objForm.DataSources.DBDataSources.Item("@GEN_CUST_BOM")
                            oDBs_Detail = objForm.DataSources.DBDataSources.Item("@GEN_CUST_BOM_D0")
                            ...
                            Dim oStatus As SAPbobsCOM.Recordset = oCompany.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
                            oStatus.DoQuery("Select Count(T1.u_status) from [@GEN_CUST_BOM] T0 inner join [@GEN_CUST_BOM_D0] T1 on T0.DocEntry=T1.DocEntry where T0.DocNum='" & DDocnum & "' and T1.u_status='DELETE' ")
                            For Row As Integer = 1 To objMatrix.VisualRowCount - (oStatus.Fields.Item(0).Value)
                                ...
                                End If
                                'oDBs_Detail.SetValue("u_itemcode", 0, "")
                                If Trim(oDBs_Detail.GetValue("u_status", oDBs_Detail.Offset)) = "DELETE" Then
                                    ''    oDBs_Detail.RemoveRecord(Row)
                                    '  objMatrix.FlushToDataSource()
                                    ...
                                    oDBs_Detail.SetValue("u_iprice", oDBs_Detail.Offset, objMatrix.Columns.Item("iprice").Cells.Item(Row).Specific.value)
