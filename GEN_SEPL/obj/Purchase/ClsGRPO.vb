        ...
        Try
            ...
            If oMr.Fields.Item(0).Value = 0 Then
                ...
                For I As Int16 = 0 To oRecordSet.RecordCount - 1
                    ...
                    Dim strItemCode As String = oRecordSet.Fields.Item("ItemCode").Value.ToString()
                    If strItemCode.Contains("'") Then
                        Dim Qr1 As String = "Select AvgPrice,OnHand  From OITW where ItemCode = '" + oRecordSet.Fields.Item("ItemCode").Value + "''''  And WhsCode = '" & oRecordSet.Fields.Item("WhsCode").Value & "'"
                        oRecordSet1.DoQuery(Qr1)
                    ...
                    Dim Qr As String = "Select AvgPrice,OnHand  From OITW where ItemCode = '" & oRecordSet.Fields.Item("ItemCode").Value & "' And WhsCode = '" & oRecordSet.Fields.Item("WhsCode").Value & "'"
                    'oRecordSet1.DoQuery("Select AvgPrice,OnHand  From OITW where ItemCode = '" & oRecordSet.Fields.Item("ItemCode").Value & "' And WhsCode = '" & oRecordSet.Fields.Item("WhsCode").Value & "'")


                    If oRecordSet1.RecordCount > 0 Then
                        ld_CurrentItemCost = Val(oRecordSet1.Fields.Item("AvgPrice").Value)
                        Dim a As Double = CDbl(oRecordSet1.Fields.Item("AvgPrice").Value)
                        Dim a1 As Double = CDbl(oRecordSet.Fields.Item(0).Value)
                        Dim a12 As String = CDbl(oRecordSet1.Fields.Item(1).Value)
                        Dim res As Double = Math.Round(a + a1 / a12, 4)
                        ld_NewItemCost = Math.Round(a + a1 / a12, 4)
                        ''ld_NewItemCost = Math.Round(CDbl(Trim(oRecordSet1.Fields.Item("AvgPrice").Value)) + (CDbl(Trim(oRecordSet.Fields.Item(0).Value)) / CDbl(Trim(oRecordSet1.Fields.Item(1).Value))), 4)

                    Else
                        ld_CurrentItemCost = 0
                        ld_NewItemCost = 0
                    End If
                    Dim con As String = ld_CurrentItemCost
                    Dim con1 As String = ld_NewItemCost
                ...
                If oSR.Add <> 0 Then
                    ...
                    ''oManual.DoQuery("select count(stacode) as code from PDN4 inner join OPDN on opdn.DocEntry=pdn4.DocEntry where OPDN.DocEntry=(Select Max(Docentry) from opdn) and NonDdctPrc='100'") ''Where U_unit = 'Unit1'

                    oManual.DoQuery("select count(stacode) as code from PDN4 inner join OPDN on opdn.DocEntry=pdn4.DocEntry where OPDN.DocEntry='33814' and NonDdctPrc='100'") ''Where U_unit = 'Unit1'
                    Dim stacode As Int32 = oManual.Fields.Item(0).Value
                    If stacode = 0 Then
                            ...
                            If oManual1.Fields.Item(0).Value > "0.00" Then
                                    ...
                                    For J As Int16 = 0 To oManual1.RecordCount - 1
                                        ...
                                        For I As Int16 = 0 To oManual.RecordCount - 1
                                            ...
                                            If oRecordSet1.RecordCount > 0 Then
                                                ...
                                                ld_NewItemCost1 = CDbl((oRecordSet1.Fields.Item("AvgPrice").Value) + ((((oManual1.Fields.Item(0).Value / 100) * (oManual.Fields.Item("Price").Value)) * (oManual.Fields.Item("Quantity").Value)) / oRecordSet1.Fields.Item(1).Value))
                                ...
                                Else
                                    ...
                                    For J As Int16 = 0 To oManual1.RecordCount - 1
                                        ...
                                        oSR1.DocDate = oManual.Fields.Item("Date").Value
                                        ''oSR1.DocDate = DateTime.ParseExact(Trim(oManual.Fields.Item("Date").Value), "yyyyMMdd", Nothing)
                                        oSR1.TaxDate = oManual.Fields.Item("Date").Value


                                        ''oSR1.DocDate = DateTime.Now
                                        ''oSR1.DocDate = DateTime.ParseExact(Trim(oManual.Fields.Item("Date").Value), "yyyyMMdd", Nothing)
                                        ''oSR1.TaxDate = DateTime.Now

                                        ''*********************************
                                        For I As Int16 = 0 To oRecordSet.RecordCount - 1
                                            'oRecordSet1.DoQuery("select price from MRV1 inner join OMRV on OMRV.docentry=MRV1.docentry where Itemcode='" + oRecordSet.Fields.Item("ItemCode").Value + "' and DocNum=(Select MAX(docnum)from omrv)")
                                            Dim strItemCode As String = oManual.Fields.Item("ItemCode").Value.ToString().Trim()
                                            If strItemCode.Contains("'") Then
                                                oRecordSet1.DoQuery("Select AvgPrice,OnHand  From OITW where ItemCode = '" & oManual.Fields.Item("ItemCode").Value & "''''  And WhsCode = '" & oManual.Fields.Item("WhsCode").Value & "'")
                                            Else
                                            ...
                                            End If
                                            ''Dim sq As String = "Select AvgPrice,OnHand  From OITW where ItemCode = '" & oManual.Fields.Item("ItemCode").Value & "''''  And WhsCode = '" & oManual.Fields.Item("WhsCode").Value & "'"
                                              ...
                                              If oRecordSet1.RecordCount > 0 Then
                                                ...
                                                ''ld_NewItemCost1 = CDbl(oRecordSet1.Fields.Item("AvgPrice").Value) + ((oManual1.Fields.Item(0).Value * oManual.Fields.Item(2).Value) / oRecordSet1.Fields.Item(1).Value)
                                            ...
                                            If ld_CurrentItemCost1 <> ld_NewItemCost1 Then
                                                ...
                                                oSR_Lines1.ItemCode = oManual.Fields.Item("ItemCode").Value
                                                oSR_Lines1.WarehouseCode = oManual.Fields.Item("WhsCode").Value
                                                Dim acc As String = oManual1.Fields.Item(1).Value
                                                oSR_Lines1.Price = ld_NewItemCost1
    ...
    Function Validation(ByVal FormUID As String) As Boolean
        Try
            ...
            For Row As Integer = 1 To objMatrix.VisualRowCount - 1
                ...
                whs.DoQuery("select itemcode,Quantity,DocNum,opdn.DocDate from OPDN inner join PDN1 on opdn.DocEntry = pdn1.DocEntry inner join ostc on pdn1.taxcode=ostc.code where ItemCode='" + objMatrix.Columns.Item("1").Cells.Item(Row).Specific.value + "' and Whscode='" + objMatrix.Columns.Item("24").Cells.Item(Row).Specific.value + "' and opdn.U_lcadd='N'  And OPDN.Docdate>='20130401' and OPDN.U_insstat='Open' and PDN1 .DocEntry not in(Select Distinct IsNUll(BaseEntry,'') From RPD1 Where BaseType = '20')")
