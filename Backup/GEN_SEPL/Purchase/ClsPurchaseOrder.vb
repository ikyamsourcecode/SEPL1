...
#End Region
    ...
    Sub LoadItems(ByVal FormUID As String, ByVal MreqNo As String)
        Try
            ...
            Dim oRecordSet As SAPbobsCOM.Recordset = oCompany.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
            oRecordSet.DoQuery("Select B.u_itemcode,B.u_totqty,A.DocNum,B.LineId From [@GEN_CUST_BOM] A Inner Join [@GEN_CUST_BOM_D0] B On A.DocEntry = B.DocEntry And B.u_cardcode = '" + Trim(objForm.Items.Item("4").Specific.value) + "' And IsNull(A.u_closed,'N') = 'N' And A.DocNum in (" + DOCNUM + ")")
